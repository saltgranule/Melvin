...
If `search` is false, the prompt is sent to the model as is, with no search context attached.

In both cases, the system prompt tells the model the current date, and asks it to keep responses short and tidy to fit Discord's 4000 character limit, to write in brief paragraphs rather than tables or graphs, and to avoid emojis unless asked for because emoji's look weird.

The model used is `openai/gpt-oss-20b`, with temperature set to 0.7.
...
- A section showing the original prompt (escaped for markdown), with a link button to [hugging face](https://huggingface.co/openai/gpt-oss-20b).
- A separator.
